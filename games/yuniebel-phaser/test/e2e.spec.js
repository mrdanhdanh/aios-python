/* e2e.spec.js — TASK-081 (bản Phaser) — npx playwright test
 * Port nguyên văn e2e.spec.js vanilla (helper: gotoGame/moveTo/hold/chaseButterfly — P3-B3).
 * Khác: URL http (Vite preview — module script không chạy file://), viewport 480×270.
 * Quy ước no-hook (P3-9): chỉ ĐỌC state qua ?test=1, KHÔNG gọi debug setter trong 2 test chơi thật.
 * Thêm: AC-7 camX() expose (P1-2), AC-12 d-pad dispatchEvent touch (P2-B5).
 */
...
// Điều khiển mèo bằng phím (port vanilla — bước lớn 120ms, nhỏ 40ms, hội tụ |dx|,|dy|≤2)
...
test("AC-2: mèo di chuyển được — hold D 1s → player.x tăng", async ({ page }) => {
  await gotoGame(page);
...
test("AC-7: camera scroll — camX()=0 khi x≤77, tăng khi x>77 (GARDEN)", async ({ page }) => {
  await gotoGame(page);
  // spawn (107,70) → camX đã > 0
  let st = await getState(page);
  let cx = await page.evaluate(() => window.__yuniebel.camX());
  expect(st.player.x).toBeGreaterThan(77);
  expect(cx).toBeGreaterThan(0);
  // di chuyển về trái (x≤77) → camX = 0
  await moveTo(page, 60, 70);
  st = await getState(page);
  cx = await page.evaluate(() => window.__yuniebel.camX());
  expect(st.player.x).toBeLessThanOrEqual(77);
  expect(cx).toBe(0);
  // di chuyển qua phải → camX tăng theo
...
test("AC-12: d-pad touch dispatchEvent → player di chuyển (P2-B5)", async ({ page }) => {
  await gotoGame(page);
  const x0 = (await getState(page)).player.x;
  await page.dispatchEvent("#pad-right", "touchstart");
  await page.waitForTimeout(600);
  await page.dispatchEvent("#pad-right", "touchend");
  const x1 = (await getState(page)).player.x;
  expect(x1).toBeGreaterThan(x0 + 5);
});

test("AC-12b: mute + ui-toggle hoạt động", async ({ page }) => {
  await gotoGame(page);
  // mute
  const muteBtn = page.locator("#mute-btn");
  await muteBtn.click();
  expect(await page.evaluate(() => window.__yuniebel.audio.isMuted())).toBe(true);
  await muteBtn.click();
  expect(await page.evaluate(() => window.__yuniebel.audio.isMuted())).toBe(false);
  // ui-toggle ẩn task box
  await page.locator("#ui-toggle").click();
  expect(await page.locator("#task-box").isVisible()).toBe(false);
  await page.locator("#ui-toggle").click();
  expect(await page.locator("#task-box").isVisible()).toBe(true);
});
...
test("AC-14a: chơi thật title → sinh nhật (D_END) — không hook", async ({ page }) => {
  ...
  if (st.phase !== "L_SEARCH") {
    await moveTo(page, 284, 70);
    await moveTo(page, 284, 52);
    st = await getState(page);
  }
  ...
  await moveTo(page, 7, 20); // lối TRÊN sofa → door_kitchen (3,30,11,20)
  st = await getState(page);
  expect(["K_INIT", "K_BLOOD", "K_CHOICE"].includes(st.phase)).toBeTruthy();

  // ===== S3 Bếp: vết máu → K_BLOOD → vùng tối → K_CHOICE =====
  st = await getState(page);
  expect(["K_INIT", "K_BLOOD", "K_CHOICE"].includes(st.phase)).toBeTruthy();
  ...
  await moveTo(page, 3, 20); // lối TRÊN sofa → door_side (2,30,11,20)
...
test("AC-14b: chơi thật title → game over (chọn 2) — không hook", async ({ page }) => {
  ...
  st = await getState(page);
  if (st.phase === "K_INIT") {
    await moveTo(page, 70, 72); // vết máu (50,78,40,8)
  }
  await page.keyboard.press("Space");
  await page.keyboard.press("Space");
  st = await getState(page);
...
test("AC-11: mood đổi đúng phase + SFX đếm được (audio.getMood/getStats)", async ({ page }) => {
  await page.goto(URL);
  ...
  // SFX cụ thể đếm tăng: di chuyển → footstepGrass
  const before = (await getStats()).footstepGrass;
  await hold(page, "d", 400);
  const after = (await getStats()).footstepGrass;
  expect(after).toBeGreaterThanOrEqual(before);
});

test("AC-13: debug API setPhase + freeze determinism (hook riêng)", async ({ page }) => {
  await page.goto(URL);
  await page.click("#btn-start");
  await page.waitForTimeout(500); // chờ fade hết (P2-B3)
  await page.evaluate(() => {
